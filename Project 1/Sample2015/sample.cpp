...
//*******************************************************************************
//initate new tools
void DrawCurve(Curve curve, GLfloat width);
...
void Display() {
	...
	//call our rotate depending on time
	RotateY(&leftStrand.p0, 180 * sinf(Time), 0, 0, 0);


	//rotate around y for our left strand
	RotateY(&leftStrand.p1, 180 * sinf(Time), 0, 0, 0);
	RotateY(&leftStrand.p2, -180 * sinf(Time), 0, 0, 0);
	...
	RotateY(&leftStrand.p3, -180 * sinf(Time), 0, 0, 0);


	//draw our left curve
	DrawCurve(leftStrand, 12);


	//rotate around y for our right strand
	RotateY(&rightStrand.p0, 180 * sinf(Time), 0, 0, 0);
	RotateY(&rightStrand.p1, 180 * sinf(Time), 0, 0, 0);
	RotateY(&rightStrand.p2, -180 * sinf(Time), 0, 0, 0);
	...
	RotateY(&rightStrand.p3, -180 * sinf(Time), 0, 0, 0);


	//draw our right curve
	DrawCurve(rightStrand, 12);
...
void DrawCurve(Curve curve, GLfloat width) {
	...
	//if our control points option is on, draw them
	if (ControlPointsOn) {
		glPushMatrix();
	...
	//for loop to create lines needed for ladder
...
void InitGraphics() {
	...
	//************************************************************************************************
	//set left strand color
	leftStrand.color(0.45, 0.45, 0.45);
	...
	//set right strand color
	rightStrand.color(0.45, 0.45, 0.45);
	...
	//************************************************************************************************
	//set bar positions, depending on numberk of bars
	float HeightBar;

	for (int i = 0; i < NUMBARS; ++i) {
		bar[i].color((float)i / NUMBARS, 1 - (float)i / NUMBARS, 0.2);
		HeightBar = (((float)i / (NUMBARS - 1)) * 2 * (HELIXHEIGHT - 0.2)) - (HELIXHEIGHT - 0.2);
		bar[i].p0.setPt(-1, HeightBar, 0);
		bar[i].p1.setPt(-1, HeightBar, 0);
		bar[i].p2.setPt(1, HeightBar, 0);
		bar[i].p3.setPt(1, HeightBar, 0);
		bar[i].reset();
	}
	//************************************************************************************************
}

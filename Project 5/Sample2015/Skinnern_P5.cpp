...
const char *WINDOWTITLE = { "Project 5 -- Nicholas Skinner" };
...
GLuint  Make_Shape;				//custom shape
...
//this will enable our vert and frag by default
...
InitLists()
{
	...
	Make_Shape = glGenLists(1);
	glNewList(Make_Shape, GL_COMPILE);
	//Make_Shape

	//this will make our teapot
	glPushMatrix();
	glRotatef(90, 0., 1., 0.);
	glColor3f(1, .5, .5);

	//glutSolidTorus(1, 4, 4, 4);
	glutSolidTeapot(1);
	glPopMatrix();
	glEndList();
...
Keyboard(unsigned char c, int x, int y)
{
	...
	case 'N':
		...
		//setup true frag and vert pattern
	case 'b':
		fragPattern = TRUE;
		vertPattern = TRUE;
		break;
		...
		//disable frag pattern
	case 'F':
		fragPattern = !fragPattern;
		break;


		//disable vert pattern
	case 'V':
